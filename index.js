...
store?.readFromFile("./alvianto_store.json");
...
global.grupbot = "120363031779586280@g.us";

//Starting In Console
async function startIchigo() {
  const { state, saveCreds } = await useMultiFileAuthState("waweb-session");
  ...
  try {
    let ichi = await makeWASocket({
      ...
      browser: ["YaSya - Bot", `Chrome`, "118.0.5993.118"],
    ...
    setInterval(() => {
      store.writeToFile("./alvianto_store.json");
    }, 60000);
    store.bind(ichi.ev);
